...
from talos_leipzig_flexbe_states.sa_approach import pose_approach
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('Refbox_Planner',
										...
										remapping={'task': 'task', 'sa_poses': 'sa_poses', 'flag_start': 'flag_start', 'waypoint': 'waypoint', 'height_sa': 'height_sa'})

			# x:528 y:96
			OperatableStateMachine.add('move_base',
										MoveBaseState(),
										transitions={'arrived': 'pose_approach', 'failed': 'failed'},
										autonomy={'arrived': Autonomy.Off, 'failed': Autonomy.Off},
										remapping={'waypoint': 'waypoint', 'flag_start': 'flag_start'})

			# x:725 y:165
			OperatableStateMachine.add('pose_approach',
										pose_approach(),
										transitions={'success': 'move_base', 'failed': 'failed'},
										autonomy={'success': Autonomy.Off, 'failed': Autonomy.Off},
										remapping={'height_sa': 'height_sa'})
